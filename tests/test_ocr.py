...
class OCRTests(unittest.TestCase):
    ...
    def test_file_selection_endpoint_accepts_multipart_and_does_not_create_copies(self) -> None:
        ...
        self.assertEqual(analysis["mensaje"], "Resultado jurídico preliminar: Conformidad.")
        self.assertEqual(analysis["revision_analista"]["estado"], "Pendiente")
        self.assertFalse(analysis["revision_analista"]["generacion_habilitada"])
    ...
    def test_analysis_endpoint_runs_ocr_and_reports_the_output_file(self) -> None:
        ...
        self.assertEqual(payload["mensaje"], "Resultado jurídico preliminar: No Conformidad.")
        self.assertEqual(payload["revision_analista"]["estado"], "Pendiente")
        ...
        self.assertEqual(progress["resultado_preliminar"], "No Conformidad")
        self.assertEqual(progress["estado_revision_analista"], "Pendiente")
    ...
    def test_official_interface_contains_simple_accessible_progress_below_analysis_button(self) -> None:
        ...
        self.assertIn("result.trazabilidad?.campos_obligatorios", interface)
        ...
        self.assertIn("id=\"review-panel\"", interface)
        self.assertIn("id=\"btn-review-confirm\"", interface)
        self.assertIn("id=\"btn-review-reject\"", interface)
        self.assertIn("Revisión del analista: Pendiente", interface)
        self.assertIn("btnGenerate.disabled = state !== 'Confirmado'", interface)
        self.assertIn("submitReview('Confirmado')", interface)
        self.assertIn("submitReview('Rechazado')", interface)
        ...
        self.assertNotIn("No fue posible extraer el texto documental.", interface)
        self.assertIn("Error en ${stage}: ${message}.", interface)
        self.assertIn("No se generaron validaciones porque el análisis se detuvo", interface)
        self.assertIn("validation.descripcion", interface)
